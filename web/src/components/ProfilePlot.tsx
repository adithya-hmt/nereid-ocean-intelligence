...
function ProfilePanel({ levels, kind, variables }: { levels: Observation[]; kind: 'best' | 'raw'; variables: string[] }) {
  ...
  const temperature = levels.filter((row) => finite(kind === 'best' ? row.temperatureBest : row.temperatureRaw))
  const salinity = levels.filter((row) => finite(kind === 'best' ? row.salinityBest : row.salinityRaw))
