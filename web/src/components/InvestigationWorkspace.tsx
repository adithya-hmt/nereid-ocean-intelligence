...
export function InvestigationWorkspace({ initialResult, benchmark = false, emptyBenchmark = false }: Props) {
  ...
  const [sectionControls, setSectionControls] = useState({ depth_step_m: 10, max_time_gap_hours: 168, max_distance_km: 500, max_vertical_gap_m: 100 })
  ...
  const submit = async () => {
    // Explicit execution owns the workspace immediately, including the AI lane.
    invalidatePlanner()
    request.current?.abort()
    invalidateSection()
  ...
  const requestSection = async () => {
    ...
    const parameters: Array<'TEMP' | 'PSAL'> = result.query_plan.parameters.length === 0 ? ['TEMP', 'PSAL'] : result.query_plan.parameters.filter((parameter): parameter is 'TEMP' | 'PSAL' => parameter === 'TEMP' || parameter === 'PSAL')
    if (parameters.length === 0) { setSectionError('A section requires requested TEMP and/or PSAL evidence.'); return }
    sectionRequest.current?.abort()
    const controller = new AbortController(); sectionRequest.current = controller
    setSectionLoading(true); setSectionError(undefined)
    const request: SectionRequest = { profile_ids: selections, qc_mode: result.query_plan.qc_mode, parameters, row_limit: result.query_plan.row_limit, ...sectionControls }
    try {
  ...
  const directSection = result?.section_request && (result.data[0] as { section_cells?: unknown[] } | undefined)?.section_cells
  const trajectoryRows = directSection ? [] : result?.data.filter(isTrajectoryRow) ?? []
  return <main className="workspace"><header className="workspace-header"><h1>Nereid Ocean Investigation</h1><p>Traceable ARGO evidence for a bounded question near 10°N, 70°E.</p></header>{benchmark ? <><TrajectoryGlobe rows={benchmarkRows} benchmark drawPoints={!emptyBenchmark} /><p className="benchmark-label">Declared browser and hardware are recorded separately from measured values in the rendering benchmark evidence.</p></> : <><QueryControls plan={plan} question={question} loading={loading} planning={planning} plannerWarning={plannerWarning} onChange={updatePlan} onQuestionChange={updateQuestion} onInterpret={interpret} onSubmit={submit} onExample={() => updatePlan(winningPlan)} /><QueryPlanPanel plan={plan} />{error && <p className="status error" role="alert">{error}</p>}{result ? <><section className="visualization" aria-labelledby="visualization-heading"><h2 id="visualization-heading">Investigation view</h2>{directSection ? null : trajectoryRows.length ? <TrajectoryGlobe key={`${trajectoryRows.length}-${trajectoryRows[0].timestamp}`} rows={trajectoryRows} /> : result.data.length ? <p className="status empty">The returned observations have no complete longitude, latitude, depth, and time coordinates for a truthful trajectory.</p> : <p className="status empty">{result.warnings[0] ?? 'No observations are available for this bounded request.'}</p>}</section>{directSection ? <><CrossSectionPlot result={result} /><ScientificReceipt result={result} selections={[]} onSelectionChange={() => undefined} readOnly /></> : <><ProfilePlot result={result} selections={selections} /><ProfileMetrics result={result} selections={selections} /><section className="section-request" aria-labelledby="section-request-heading"><h2 id="section-request-heading">Derived cross-section</h2><p>Uses exactly the selected representations; unsupported gaps remain masked.</p><div className="control-grid">{([{ key: 'depth_step_m', label: 'Depth step (m)', min: 1, max: 100 }, { key: 'max_time_gap_hours', label: 'Maximum time gap (hours)', min: 1, max: 744 }, { key: 'max_distance_km', label: 'Maximum distance (km)', min: 1, max: 2000 }, { key: 'max_vertical_gap_m', label: 'Maximum vertical gap (m)', min: 1, max: 500 }] as const).map(({ key, label, min, max }) => <label key={key}>{label}<input type="number" min={min} max={max} value={sectionControls[key]} onChange={(event) => { invalidateSection(); setSectionControls({ ...sectionControls, [key]: Number(event.target.value) }) }} /></label>)}</div><button className="primary-button" type="button" disabled={selections.length < 2 || sectionLoading || (result.query_plan.parameters.length > 0 && !result.query_plan.parameters.some((parameter) => parameter === 'TEMP' || parameter === 'PSAL'))} onClick={() => void requestSection()}>{sectionLoading ? 'Deriving section…' : 'Derive section from selected representations'}</button>{selections.length < 2 && <p role="status">Select at least two exact representations before requesting a section.</p>}{result.query_plan.parameters.length > 0 && !result.query_plan.parameters.some((parameter) => parameter === 'TEMP' || parameter === 'PSAL') && <p role="status">A section requires requested TEMP and/or PSAL evidence.</p>}{sectionError && <p className="status error" role="alert">{sectionError}</p>}</section>{section && <CrossSectionPlot result={section} />}{section && <ScientificReceipt result={section} selections={[]} onSelectionChange={() => undefined} readOnly heading="Derived section receipt" headingId="derived-section-receipt-heading" />}<ScientificReceipt result={result} selections={selections} onSelectionChange={updateSelections} /></>}</> : <section className="visualization empty" aria-live="polite"><h2>Investigation view</h2><p>Run the bounded query to inspect source-faithful observations and their receipt.</p></section>}</>}</main>
}
