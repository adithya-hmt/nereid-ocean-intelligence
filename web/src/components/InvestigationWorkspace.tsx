...
import { ApiError, executeQuery, interpretQuestion } from '../lib/api'
...
export function InvestigationWorkspace({ initialResult, benchmark = false, emptyBenchmark = false }: Props) {
  ...
  const interpret = async () => {
    ...
    try {
      const response = await interpretQuestion(question)
      if (response.plan) setPlan(response.plan)
      setPlannerWarning(response.warnings[0])
    } catch (caught) {
      setPlannerWarning(caught instanceof ApiError ? `AI interpretation unavailable—filters still work. ${caught.message}` : 'AI interpretation unavailable—filters still work. Use explicit filters.')
    } finally {
      setPlanning(false)
    }
  }
