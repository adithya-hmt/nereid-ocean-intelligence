...
const result = (wmo: string, cycle: number, source_profile_index: number) => ({
  ...
  data: [{ wmo, cycle, source_profile_index, vertical_sampling_scheme: 'primary' }], chart_spec: [], provenance: [], qc_summary: { retained: 1, rejected: 0 }, methods: [], assumptions: [], warnings: [],
})

function Receipt({ current }: { current: ReturnType<typeof result> }) {
  const [selected, setSelected] = useState([{ wmo: current.data[0].wmo, cycle: current.data[0].cycle, source_profile_index: current.data[0].source_profile_index }])
