...
const requestedParameters = (result: ResultEnvelope) => {
  const parameters = result.section_request?.parameters ?? result.query_plan.parameters.filter((parameter): parameter is 'TEMP' | 'PSAL' => parameter === 'TEMP' || parameter === 'PSAL')
  return parameters.length ? parameters : ['TEMP', 'PSAL']
}
...
export function CrossSectionPlot({ result }: { result: ResultEnvelope }) {
  ...
  const parameters = requestedParameters(result)
  const hasTemperature = parameters.includes('TEMP')
  const hasSalinity = parameters.includes('PSAL')
  const sectionName = hasTemperature && hasSalinity ? 'temperature and salinity' : hasTemperature ? 'temperature' : 'salinity'
  ...
  const status = (cell: SectionCell) => {
    if ((hasTemperature && cell.temperature !== null) || (hasSalinity && cell.salinity !== null)) return 'interpolated derived'
    const reason = cell.mask_reason ?? gaps.get(gapKey(cell))
    return reason ? reason.replaceAll('_', ' ') : 'masked'
  }

  return <section className="section-plot" aria-labelledby="section-heading"><h2 id="section-heading">Gap-masked {sectionName} cross-section</h2><p>Cells across unsupported observations remain blank; values are never smoothed across a gap.</p><svg viewBox="0 0 320 120" role="img" aria-label={`Gap-masked ${sectionName} cross-section`}>{cells.map((cell) => <rect key={cellKey(cell)} x={x(cell)} y={y(cell.depth_m)} width={profileSpacing} height={cellHeight} className={(hasTemperature && cell.temperature !== null) || (hasSalinity && cell.salinity !== null) ? 'section-cell' : 'masked-cell'} />)}{coordinates.map((coordinate, index) => <circle key={`${coordinate.wmo}-${coordinate.cycle}-${coordinate.source_profile_index}-${coordinate.vertical_sampling_scheme}-${coordinate.latitude}-${coordinate.longitude}-${coordinate.timestamp}`} cx={left + index * profileSpacing} cy="112" r="4" className="observation-marker"><title>{coordinateTitle(coordinate)}</title></circle>)}</svg><table aria-label="Cross-section observations"><caption>Cross-section observations</caption><thead><tr><th>Depth</th><th>Between representations</th>{hasTemperature && <th>Temperature</th>}{hasSalinity && <th>Salinity</th>}<th>Status</th></tr></thead><tbody>{cells.map((cell) => <tr key={cellKey(cell)}><td>{cell.depth_m} m</td><td>{representations(cell)}</td>{hasTemperature && <td>{cell.temperature === null ? 'masked' : `${cell.temperature.toFixed(2)} °C`}</td>}{hasSalinity && <td>{cell.salinity === null ? 'masked' : `${cell.salinity.toFixed(3)} g kg⁻¹`}</td>}<td>{status(cell)}</td></tr>)}</tbody></table></section>
}
