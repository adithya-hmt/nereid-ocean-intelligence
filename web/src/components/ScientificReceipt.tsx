...
type Props = { result: ResultEnvelope; selections: Selection[]; onSelectionChange: (selections: Selection[]) => void; readOnly?: boolean; heading?: string; headingId?: string }
...
export function ScientificReceipt({ result, selections, onSelectionChange, readOnly = false, heading = 'Scientific receipt', headingId = 'receipt-heading' }: Props) {
  ...
  const sectionRequest = readOnly ? result.section_request : undefined
  return <section className="receipt" aria-labelledby={headingId}><h2 id={headingId}>{heading}</h2>{sectionRequest && <div className="section-receipt-evidence"><h3>Section request</h3><p>Exact representations: {sectionRequest.profile_ids.map((item) => `${item.wmo} / cycle ${item.cycle} / ${item.direction} / representation ${item.source_profile_index}`).join('; ')}</p><dl>{Object.entries(sectionRequest).filter(([name]) => name !== 'profile_ids').map(([name, value]) => <div key={name}><dt>{name}</dt><dd>{parameterValue(value)}</dd></div>)}</dl></div>}{!readOnly && <><fieldset><legend>Select exact profile representations for linked views and evidence export</legend>{representations.map((item) => <label key={`${item.wmo}/${item.cycle}/${item.direction}/${item.source_profile_index}`}><input type="checkbox" checked={selections.some((choice) => same(choice, item))} onChange={() => toggle(item)} />{item.wmo} / cycle {item.cycle} / {item.direction} / representation {item.source_profile_index} ({item.scheme})</label>)}</fieldset><button type="button" onClick={() => void download()} disabled={!selections.length}>Download evidence ZIP</button></>}<div className="receipt-counts"><span><strong>{result.qc_summary.retained}</strong> retained</span><span><strong>{result.qc_summary.rejected}</strong> excluded</span></div><div className="receipt-columns"><div><h3>Sources</h3>{result.provenance.map((source) => <p key={source.sha256}><a href={source.source_url}>Source file</a><br />DOI {source.snapshot_doi}<br />Fetched <time dateTime={source.fetched_at}>{source.fetched_at}</time><br /><code>{source.sha256}</code></p>)}</div><div><h3>Methods</h3>{result.methods.map((method) => <div key={method.name}><p>{method.name}</p><dl>{Object.entries(method.parameters).map(([name, value]) => <div key={name}><dt>{name}</dt><dd>{parameterValue(value)}</dd></div>)}{Object.entries(method.units ?? {}).map(([name, unit]) => <div key={`unit-${name}`}><dt>{name} units</dt><dd>{unit}</dd></div>)}</dl></div>)}</div><div><h3>Assumptions & warnings</h3>{[...result.assumptions, ...result.warnings].map((item) => <p key={item}>{item}</p>)}</div></div></section>
}
