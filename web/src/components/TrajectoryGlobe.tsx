...
type BenchmarkMetrics = { initializationMs?: number; r3fFps?: number; browser?: string; hardware?: string }
...
function PointCloud({ rows, exaggeration }: { rows: readonly TrajectoryRow[]; exaggeration: number }) {
  ...
  return <points><bufferGeometry><bufferAttribute attach="attributes-position" args={[buffers.positions, 3]} /><bufferAttribute attach="attributes-color" args={[buffers.colors, 3]} /></bufferGeometry><pointsMaterial vertexColors size={0.003} sizeAttenuation /></points>
}

function FrameProbe({ benchmark, startedAt, onFirstFrame, onComplete }: { benchmark: boolean; startedAt: { current: number }; onFirstFrame: (initializationMs: number) => void; onComplete: (fps: number) => void }) {
  const invalidate = useThree((state) => state.invalidate)
  const frameState = useRef({ seen: false, sampleStartedAt: 0, frames: 0, complete: false })
  useFrame(() => {
    ...
    if (elapsed >= 5000) {
      state.complete = true
      onComplete(state.frames / (elapsed / 1000))
      return
    }
    invalidate()
  })
...
export function TrajectoryGlobe({ rows, benchmark = false }: Props) {
  ...
  const recordComplete = (r3fFps: number) => setMetrics((current) => ({ ...current, r3fFps }))
  return <section className="trajectory-view" aria-labelledby="trajectory-heading" data-render-ready={ready ? 'true' : undefined} data-render-benchmark-complete={metrics.r3fFps ? 'true' : undefined}>
    ...
    {fallback ? <TrajectoryFallback rows={visibleRows} label={reducedMotion ? '2D longitude/latitude fallback (reduced motion)' : webglFailed ? '2D longitude/latitude fallback (WebGL unavailable)' : undefined} /> : <WebglBoundary onError={() => setWebglFailed(true)}><div className="globe-canvas"><Canvas frameloop="demand" camera={{ position: [0, 0, 2.6], fov: 45 }} onCreated={({ gl }) => { startedAt.current = performance.now(); gl.setClearColor('#f4f7f4') }}><ambientLight intensity={1} /><PointCloud rows={visibleRows} exaggeration={exaggeration} /><FrameProbe benchmark={benchmark} startedAt={startedAt} onFirstFrame={recordFirstFrame} onComplete={recordComplete} /></Canvas></div></WebglBoundary>}
