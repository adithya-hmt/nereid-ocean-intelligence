...
export function QueryControls({ plan, question, loading, planning, plannerWarning, onChange, onQuestionChange, onInterpret, onSubmit, onExample }: Props) {
  ...
  const updateParameters = (parameter: QueryPlan['parameters'][number]) => onChange({ ...plan, parameters: plan.parameters.includes(parameter) ? plan.parameters.filter((item) => item !== parameter) : [...plan.parameters, parameter] })
  ...
  return <form className="query-controls" onSubmit={(event) => { event.preventDefault(); onSubmit() }}>
    ...
    <div className="control-grid">
      ...
      {(['TEMP', 'PSAL', 'PRES'] as const).map((parameter) => <label key={parameter}><input type="checkbox" checked={plan.parameters.includes(parameter)} onChange={() => updateParameters(parameter)} />{parameter}</label>)}
