...
def test_normalization_preserves_provenance_and_best_values(
    argo_nc, tmp_path, source_manifest
):
    ...
    assert levels["temperature_adjusted_error"].tolist() == [
        0.01,
        0.02,
        0.03,
        0.04,
    ...
    assert levels["salinity_adjusted_error"].tolist() == [
        0.001,
        0.002,
        0.003,
        0.004,
    ...
    assert levels["pressure_raw"].tolist() == [0, 10, 20, 30, 40, 50]
    assert levels["pressure_adjusted"].tolist() == [0.1, 10.1, 20.1, 30.1, 40.1, 50.1]
    assert levels["pressure_adjusted_qc"].tolist() == [1, 1, 1, 4, 1, 1]
    assert levels["pressure_adjusted_error"].tolist() == [0.1] * 6
    expected_sa = gsw.SA_from_SP(34.01, 0.1, 70.0, 10.0)
    expected_ct = gsw.CT_from_t(expected_sa, 27.9, 0.1)
    assert np.isclose(levels.loc[0, "absolute_salinity"], expected_sa, rtol=1e-10)
    assert np.isclose(
        levels.loc[0, "conservative_temperature"], expected_ct, rtol=1e-10
    )


def test_normalization_preserves_duplicate_cycle_representations(
    argo_nc_with_duplicate_profile_key, tmp_path
):
    ...
    assert result.profiles["vertical_sampling_scheme"].to_pylist() == [
        "unspecified",
        "unspecified",
    ]
...
def test_normalization_is_idempotent(argo_nc, tmp_path, source_manifest):
    ...
    assert (first.profile_count, first.level_count) == (
        second.profile_count,
        second.level_count,
    )
